...
class Computer {
  constructor(register, instructions) {
    ...
    this.startingRegister = Object.values(register);
    this.instructions = instructions;
    this.output = [];
    this.pointer = 0;
    // add candidate check flag
    this.check = [2, 4, 1, 5, 7, 5, 1, 6, 0, 3, 4, 1, 5, 5, 3, 0];
  ...
  run() {
    while (this.pointer <= this.instructions.length) {
      // break if not candidate
      if (this.isCandidate === false) break;
      else if (this.check.length === 0 && this.isCandidate) {
        console.log("CANDIDATE FOUND");
        console.log(this.startingRegister);
        break;
      }
...
for (let i = 35184372088832; i < 281474976710655; i++) {
  let testRegister = {
    A: i,
    B: 0,
    C: 0,
  ...
  let testComputer = new Computer(testRegister, programArr);
  testComputer.run();
}
